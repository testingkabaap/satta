<html lang="en">

<!-- Added by HTTrack -->
<meta http-equiv="content-type" content="text/html;charset=UTF-8" /><!-- /Added by HTTrack -->

<head>
  <title>BBL Satta</title>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link href="<?php base_url('assets/plugins/bootstrap/bootstrap.min.css'); ?>" rel="stylesheet">
  <link rel="stylesheet" href="<?php base_url('assets/css/new.css') ?>">
  <meta name="robots" content="">
  <style>
    body {
      background-color: #f5f9d0 !important;
    }
  </style>
</head>







<body style="background-color:#eef2009e !important;">

  <section class="head_marquee home">
    <img class="ban_img" src="<?php base_url('assets/images/sattaBanner.jpg') ?>">
    <div class="container-fluid">
      <div class="heading">
        <h2>
          WELCOME
          <br>
          TO
          <br>
          <strong>
            BBL Lottry Satta
          </strong>
        </h2>

      </div>
      <marquee onmouseover="this.stop();" onmouseout="this.start();">
        <h2>
          बी.बी.एल रियल सट्टा में आपका स्वागत है
        </h2>

      </marquee>
      <h3 id="demo"></h3>
    </div>

  </section>

  <div class="latest_result">
    <div>
      <h2>Today Latest Result</h2>
      <h3>BBL Lottry Satta</h3>
      <p>(Time: 10:20 AM)</p>
      <p id="last_resutl_time" style="display:none;">12:20:00</p>
      <div class="d-flex justify-content-center">
        <div>
          <center><img src="<?php echo base_url('assets/images/down.gif'); ?>"></center>
          <div class="number_lottry align-items-center" style="font-size: 40px; font-weight: bold; background: red; color: #fff;">

            <span>
              36 </span>
          </div>
          <center><img src="<?php echo base_url('assets/images/new2.gif'); ?>" width="30%"></center>
        </div>
      </div>
      <!-- <p>Result out in <strong id="current-time"></strong></p> -->
    </div>
  </div>

  <div class="card-row" id="cardContainer">



    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 09:00 AM)</p>
        <div class="time">

          <span id="yesterday">
            81
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            02 </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 09:20 AM)</p>
        <div class="time">

          <span id="yesterday">
            82
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            48 </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 09:40 AM)</p>
        <div class="time">

          <span id="yesterday">
            65
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            98 </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 10:00 AM)</p>
        <div class="time">

          <span id="yesterday">
            93
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            25 </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 10:20 AM)</p>
        <div class="time">

          <span id="yesterday">
            75
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            36 </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 10:40 AM)</p>
        <div class="time">

          <span id="yesterday">
            87
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 11:00 AM)</p>
        <div class="time">

          <span id="yesterday">
            95
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 11:20 AM)</p>
        <div class="time">

          <span id="yesterday">
            69
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 11:40 AM)</p>
        <div class="time">

          <span id="yesterday">
            39
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 12:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            66
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 12:20 PM)</p>
        <div class="time">

          <span id="yesterday">
            68
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 12:40 PM)</p>
        <div class="time">

          <span id="yesterday">
            63
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 01:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            09
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 01:20 PM)</p>
        <div class="time">

          <span id="yesterday">
            11
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 01:40 PM)</p>
        <div class="time">

          <span id="yesterday">
            16
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 02:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            07
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 02:20 PM)</p>
        <div class="time">

          <span id="yesterday">
            80
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 02:40 PM)</p>
        <div class="time">

          <span id="yesterday">
            75
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 03:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            96
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 03:20 PM)</p>
        <div class="time">

          <span id="yesterday">
            34
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 03:40 PM)</p>
        <div class="time">

          <span id="yesterday">
            77
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 04:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            67
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 04:20 PM)</p>
        <div class="time">

          <span id="yesterday">
            72
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 04:40 PM)</p>
        <div class="time">

          <span id="yesterday">
            86
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 05:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            98
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 05:20 PM)</p>
        <div class="time">

          <span id="yesterday">
            07
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 05:40 PM)</p>
        <div class="time">

          <span id="yesterday">
            15
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 06:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            86
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 06:20 PM)</p>
        <div class="time">

          <span id="yesterday">
            32
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 06:40 PM)</p>
        <div class="time">

          <span id="yesterday">
            38
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 07:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            36
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 07:20 PM)</p>
        <div class="time">

          <span id="yesterday">
            39
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 07:40 PM)</p>
        <div class="time">

          <span id="yesterday">
            79
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 08:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            14
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 08:20 PM)</p>
        <div class="time">

          <span id="yesterday">
            19
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 08:40 PM)</p>
        <div class="time">

          <span id="yesterday">
            08
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 09:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            15
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #fff">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 09:20 PM)</p>
        <div class="time">

          <span id="yesterday">
            13
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #ffd700">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 09:40 PM)</p>
        <div class="time">

          <span id="yesterday">
            80
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>




    <div class="result_box_card" style="background: #cdcdcd">
      <div>
        <h2>BBL SATTA</h2>
        <p>(Time: 10:00 PM)</p>
        <div class="time">

          <span id="yesterday">
            87
          </span>
          <img src="<?php echo base_url('assets/images/new2.gif'); ?>">
          <span id="today" class="new">
            XX </span>

        </div>
        <a href="result7956.html?date=2024-01-15" class="view_chart">View Chart</a>
      </div>
    </div>

  </div>
  <section class="news_letter">
    <marquee>
      <div class="emai_wrap">
        <a href="https://api.whatsapp.com/send?phone=Only%20for%20WhatsApp" class="whatsapp_icon" target="_blank" rel="noopener noreferrer">
          Contact Us : +91-Only for WhatsApp (Only Whatsaap)
        </a>
        <a href="#" target="_blank">
          Email : bblsattalottarysatta@gmail.com </a>
      </div>
    </marquee>

  </section>
  <section>
    <div class="container">
      <div class="row">
        <div class="col-md-3">
          <a href="#" target="_blank">BBL SATTA</a>
        </div>
        <div class="col-md-3">
          <a href="#" target="_blank">BBL SATTA</a>
        </div>
        <div class="col-md-3">
          <a href="#" target="_blank">BBL SATTA</a>
        </div>
        <div class="col-md-3">
          <a href="#" target="_blank">BBL SATTA</a>
        </div>
      </div>
    </div>
  </section>
</body>

</html>